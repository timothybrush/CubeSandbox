...
setup_env() {
	...
	CUBE_IMAGE_TAG="${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.5.1-rc10}"
...
build_and_push_images() {
	...
	user=$(terraform output -raw tcr_token_user 2>/dev/null || echo "")
	tag="${CUBE_IMAGE_TAG:-v0.5.1-rc10}"
...
tcr_build_and_push() {
	...
	echo -e "  ${GREEN}✓ Image tag to build & push: ${CUBE_IMAGE_TAG:-v0.5.1-rc10}${NC}"
	echo ""
...
prompt_deployment_env() {
	...
	select_env TENCENTCLOUD_CUBE_IMAGE_TAG "Cube component image tag" "v0.5.1-rc10" "dev"
...
TENCENTCLOUD_CUBE_IMAGE_TAG='${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.5.1-rc10}'
...
write_resolved_tfvars_file() {
	...
	jq -n \
		...
		--arg image_tag "${TF_VAR_image_tag:-${CUBE_IMAGE_TAG:-${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.5.1-rc10}}}" \
